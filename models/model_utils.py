...
class _CoreResNet(nn.Module):
    def __init__(self, block, layers, inplanes, outplanes, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block, outplanes[0], layers[0], stride=1)
        self.layer2 = self._make_layer(block, outplanes[1], layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, outplanes[2], layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, outplanes[3], layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, _ResnetBottleneck):
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, _ResnetBasicBlock):
...
class ConvBlockCircular(nn.Module):
    """
    Convolution with circular padding along feature dimension (last dimension)
    """
    def __init__(self, in_channels, out_channels):
    ...
    def forward(self, input, pool_size=(2, 2), pool_type='avg'):
        ...
        """
        x = input
        # circular pading, pytorch pad from last dimension to first dimension
